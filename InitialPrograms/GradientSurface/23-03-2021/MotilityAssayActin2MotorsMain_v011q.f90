PROGRAM MAIN
	...
	!USE PLANAR_SURFACE
	!USE CHANNEL_TRACK
	!USE PLANAR_SURFACE_yGRADIENT
	USE PLANAR_SURFACE_STEP
	...
	CALL FilamentInitialConfigurations(IniAngle,REAL(NumBeads-1,DP)*BondLength,X,Y,Z)
...
!		CALL OutputMotorStates(UnitNumMotorStates, TS, ActiveMotorIdxOffset, ActiveMotorIdxEnd, X, Y, Z, XM, YM, ZM, F_Motor_X, F_Motor_Y, F_Motor_Z, ContactState, MotorType)
	...
	IF (DABS(Y(1,1)) > 0.5_DP*ChannelWidth) EXIT Loop_Time				! Gradient
	IF (Z(1,1) > REAL(NumBeads - 1)*BondLength) EXIT Loop_Time			! Gradient
