...
!---Initial Conformation of Filaments-----------------------------
	...
	SUBROUTINE FilamentInitialConfigurations(IniAngle,LengthToTarget,X,Y,Z)

		USE PARAMETERS , ONLY : DP, NumFilament, NumBeads, BondLength
		
		
		REAL(KIND = DP), INTENT(IN) :: IniAngle, LengthToTarget
		
		REAL(KIND = DP), DIMENSION(NumBeads, NumFilament), INTENT(OUT) :: X, Y, Z
		...
		DO I=1, NumFilament
			...
			X(1,I) = -LengthToTarget*DCOS(IniAngle)
			Y(1,I) = -LengthToTarget*DSIN(IniAngle)
			Z = 0.0125_DP
		
			DO J=1, NumBeads-1
		
				X(J+1,I) = X(J,I) - BondLength*DCOS(IniAngle)
				Y(J+1,I) = Y(J,I) - BondLength*DSIN(IniAngle)
		
			END DO
		END DO


	END SUBROUTINE FilamentInitialConfigurations
