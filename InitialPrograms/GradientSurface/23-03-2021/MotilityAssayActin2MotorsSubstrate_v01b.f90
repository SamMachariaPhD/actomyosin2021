...
!---Initial Motor Location----------------------------------------------------------------
	...
	!---Initial Motor Location----------------------------------------------------------------
		...
		SUBROUTINE InitialMotorLocations(ActiveMotorIdxOffset, ActiveMotorIdxEnd, IniAngle, X, Y, XM, YM, ZM, ContactState, AddedMotorNum, AreaCounter, AreaOriginX, AreaOriginY, AreaOriginUx, AreaOriginUy, MotorType)
			...
			AddedMotorCounter = 0
			DO IM=1, NINT(MAX(Motor_Density_Upper,Motor_Density_Lower)*HorizontalLength*VerticalLength, Range15)
		...
		FUNCTION Prob_yLinearGrad(Coordinate)
			...
			IF (Motor_Density_Upper >= Motor_Density_Lower) THEN
				Prob_yLinearGrad = 0.5_DP*(1.0_DP + Motor_Density_Lower/Motor_Density_Upper) + (1.0_DP - Motor_Density_Lower/Motor_Density_Upper)*Coordinate/ChannelWidth
			ELSE
				Prob_yLinearGrad = 0.5_DP*(1.0_DP + Motor_Density_Upper/Motor_Density_Lower) + (Motor_Density_Upper/Motor_Density_Lower - 1.0_DP)*Coordinate/ChannelWidth
			END IF
...
!					IF (DABS(YM_New) > 0.5_DP*ChannelWidth) CYCLE
					...
					UR1 = grnd()
					IF ((InNewArea == .TRUE.) .AND. (UR1 <= Prob_yStep(YM_New))) THEN
			...
			FUNCTION Prob_yStep(Coordinate)
		
		
				USE PARAMETERS, ONLY : DP, Motor_Density_Upper, Motor_Density_Lower, ChannelWidth
			
			
				REAL(KIND = DP) :: Prob_yStep
				...
				IF (Motor_Density_Upper >= Motor_Density_Lower) THEN
					IF (Coordinate >= 0.0_DP) THEN
						Prob_yStep = 1.0_DP
					ELSE
						Prob_yStep = Motor_Density_Lower/Motor_Density_Upper
					END IF
				ELSE
					IF (Coordinate >= 0.0_DP) THEN
						Prob_yStep = Motor_Density_Upper/Motor_Density_Lower
					ELSE
						Prob_yStep = 1.0_DP
					END IF
				END IF
			...
			END FUNCTION Prob_yStep
	
	
	
			
			END MODULE PLANAR_SURFACE_STEP
