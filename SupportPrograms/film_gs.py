...
Filament_file_list = glob.glob('Filament_A**.vtk') # all files starting with 'Filament' and ending with '.vtk'
...
MTPlusEnd_file_list = glob.glob('FilamentPlusEnd**.vtk')

filament_files = sorted(Filament_file_list, key=lambda x:x[-8:]) # sort by the last 16 characters of the file name
IntSpecie1_files = sorted(IntSpecie1_file_list, key=lambda x:x[-8:])
IntSpecie2_files = sorted(IntSpecie2_file_list, key=lambda x:x[-8:])
MotorSpecie1_files = sorted(MotorSpecie1_file_list, key=lambda x:x[-8:])
MotorSpecie2_files = sorted(MotorSpecie2_file_list, key=lambda x:x[-8:])
MTPlusEnd_files = sorted(MTPlusEnd_file_list, key=lambda x:x[-8:])
