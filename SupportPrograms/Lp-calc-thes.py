...
ax[0].plot(xy0[0]['x'], xy0[0]['y'], color='red') # Original trajectory
ax[0].plot(conf0[0]['x'], conf0[0]['y'], color='blue') # Trajectory after jump
ax[0].minorticks_on()
ax[0].tick_params('both', direction='in', length=8, top=True, right=True, which='major')
ax[0].tick_params('both', direction='in', length=4, top=True, right=True, which='minor')
...
ax[1].minorticks_on()
ax[1].tick_params('both', direction='in', length=8, top=True, right=True, which='major')
ax[1].tick_params('both', direction='in', length=4, top=True, right=True, which='minor')
ax[1].set_xlabel('X($\mu m$)')

ax[2].plot(xy0[2]['x'], xy0[2]['y'], color='red')
ax[2].plot(conf0[2]['x'], conf0[2]['y'], color='blue')
ax[2].minorticks_on()
ax[2].tick_params('both', direction='in', length=8, top=True, right=True, which='major')
ax[2].tick_params('both', direction='in', length=4, top=True, right=True, which='minor')
...
#         print("Ds = %s"%i, file=open('ds_ub_status.txt','a')) # For troubleshooting
...
for i in range(0,30): # 30 trajectory plots
    ax.plot(s0_m,dfdsm0[i], marker='o', markersize=3, ls='--', lw=1, color='lightblue', markerfacecolor='lime')
    
ax.plot(s0_m,dsm0_m, marker='o', markersize=3, ls='--', lw=1, color='red', markerfacecolor='lime') # Mean plot
